...
# Functions ##################################################################
##############################################################################
#---> Color coded message functions with timestamps
...
# Additional settings not normally present
sudo grep -q "^MaxStartups" /etc/ssh/sshd_config || echo "MaxStartups 10:30:60" | sudo tee -a /etc/ssh/sshd_config > /dev/null
sudo grep -q "^AuthenticationMethods" /etc/ssh/sshd_config || echo "AuthenticationMethods publickey" | sudo tee -a /etc/ssh/sshd_config > /dev/null

# Add crypto settings (these are usually not in default config)
sudo grep -q "^Ciphers" /etc/ssh/sshd_config || echo "Ciphers chacha20-poly1305@openssh.com,aes256-gcm@openssh.com,aes256-ctr" | sudo tee -a /etc/ssh/sshd_config > /dev/null
